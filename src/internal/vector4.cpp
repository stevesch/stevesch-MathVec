...
#define LOCAL_UNINIT(type, ptr, count) type *ptr = ((type *)SALLOCA(count * sizeof(type)))
  ...
  float vector4::distanceSquared(vector4 &vNearestPoint, const vector4 *pTriangle, int *pnNearestEdge) const
  {
    ...
    //		vector4::sub(e[0], pTriangle[2], pTriangle[1]);
    //		vector4::sub(e[1], pTriangle[0], pTriangle[2]);
    //		vector4::sub(e[2], pTriangle[1], pTriangle[0]);

    //		vector4::sub(r[0], *this, pTriangle[0]);
    //		vector4::sub(r[1], *this, pTriangle[1]);
    //		vector4::sub(r[2], *this, pTriangle[2]);
    vector4::cross(vNormal, e[1], e[2]);

    do
    {
      float t, tDenom;

      vector4::cross(c, e[2], vNormal);
      f0 = r[0].dot(c);
      if (f0 > 0.0f)
      {
      ...
      // p' = p - n*[ (p - v[0]) . n] / (n . n)
      t = -r[0].dot(vNormal);
      tDenom = vNormal.dot(vNormal);
      vector4::addScaled(vNearestPoint, *this, vNormal, t / tDenom);
    ...
    return squareDist(vNearestPoint, *this);
  }
                                   ...
                                   float *pfSParam, float *pfTParam) const
  {
    ...
    float fA00 = e0.squareMag();
    float fA01 = e0.dot(e1);
    float fA11 = e1.squareMag();
    float fB0 = kDiff.dot(e0);
    float fB1 = kDiff.dot(e1);
    float fC = kDiff.squareMag();
  ...
  bool vector4::inTriangle(const vector4 *pTri, const vector4 &vNormal) const
  {
    ...
    vector4::sub(rq, *this, pTri[0]);
    vector4::sub(vEdgeNormal, pTri[0], pTri[1]);
    vEdgeNormal.cross(vNormal);
    if (rq.dot(vEdgeNormal) < 0.0f)
      return false;

    vector4::sub(rq, *this, pTri[1]);
    vector4::sub(vEdgeNormal, pTri[1], pTri[2]);
    vEdgeNormal.cross(vNormal);
    if (rq.dot(vEdgeNormal) < 0.0f)
      return false;

    vector4::sub(rq, *this, pTri[2]);
    vector4::sub(vEdgeNormal, pTri[2], pTri[0]);
    vEdgeNormal.cross(vNormal);
    if (rq.dot(vEdgeNormal) < 0.0f)
      return false;
  ...
  bool vector4::inTriangle(const vector4 *pTri) const
  {
		...
		vector4::sub( vEdge, pTri[1], pTri[0] );
		vector4::sub( aq, *this, pTri[0] );
		aq.cross( vEdge );	// aq x ab

		vector4::sub( vEdge, pTri[2], pTri[1] );
		vector4::sub( rq, *this, pTri[1] );
		rq.cross( vEdge );	// bq x bc
		if (aq.dot(rq) < 0.0f)	// (aq x ab).(bq x bc) < 0
			return false;
		
		vector4::sub( vEdge, pTri[0], pTri[2] );
		vector4::sub( rq, *this, pTri[2] );
		rq.cross( vEdge );	// cq x ca
		if (aq.dot(rq) < 0.0f)	// (aq x ab).(cq x ca) < 0
			return false;
    ...
    vector4::sub(vEdge[0], pTri[1], pTri[0]);    // edge 0
    vector4::sub(vEdge[1], pTri[2], pTri[1]);    // edge 1
    vector4::sub(vEdge[2], pTri[0], pTri[2]);    // edge 2
    vector4::cross(vNormal, vEdge[2], vEdge[0]); // normal = (edge2 x edge0)

    vector4::cross(vEdgeNormal, vNormal, vEdge[0]);
    vector4::sub(rq, *this, pTri[0]);
    if (rq.dot(vEdgeNormal) < 0.0f)
      return false;

    vector4::cross(vEdgeNormal, vNormal, vEdge[1]);
    vector4::sub(rq, *this, pTri[1]);
    if (rq.dot(vEdgeNormal) < 0.0f)
      return false;

    vector4::cross(vEdgeNormal, vNormal, vEdge[2]);
    vector4::sub(rq, *this, pTri[2]);
    if (rq.dot(vEdgeNormal) < 0.0f)
      return false;
    ...
    vector4::sub(v1, pTri[0], pTri[2]);

    for (i = 0; i < 3; i++)
    {
      float scale;
      int idx = ((i == 2) ? 0 : i + 1);
      vector4::sub(v2, pTri[idx], pTri[i]);
      vector4::sub(r, *this, pTri[i]);

      scale = v1.dot(v2) / v1.dot(v1);
      v1 *= scale;
      vector4::sub(v1, v2, v1);
  ...
  bool vector4::inTriangleEx(const vector4 *pEdges, const vector4 *pVectors, const vector4 &vNormal) const
  {
    ...
    //		vector4::cross( vNormal, pEdges[2], pEdges[0] );		// normal = (edge2 x edge0)

    vector4::cross(vEdgeNormal, vNormal, pEdges[0]);
    if (pVectors[0].dot(vEdgeNormal) < 0.0f)
      return false;

    vector4::cross(vEdgeNormal, vNormal, pEdges[1]);
    if (pVectors[1].dot(vEdgeNormal) < 0.0f)
      return false;

    vector4::cross(vEdgeNormal, vNormal, pEdges[2]);
    if (pVectors[2].dot(vEdgeNormal) < 0.0f)
      return false;
                                  ...
                                  float fZeroOffset)
  {
    ...
    vSquareDist.x = d0.squareMag();
    vSquareDist.y = d1.squareMag();
    vSquareDist.z = d2.squareMag();
    vSquareDist.w = d3.squareMag();
                               ...
                               const vector4 &v3)
  {
    ...
    vSquareDist.x = r0.squareMag();
    vSquareDist.y = r1.squareMag();
    vSquareDist.z = r2.squareMag();
    vSquareDist.w = r3.squareMag();
  }
      ...
      const vector4 &vAScale0, const vector4 &vAScale1, float dt)
  {
    ...
    vector4::addScaled(v0, v0, a0, vAScale.x);
    vector4::addScaled(v1, v1, a1, vAScale.y);
    vector4::addScaled(v2, v2, a2, vAScale.z);
    vector4::addScaled(v3, v3, a3, vAScale.w);

    vector4::addScaled(r0, r0, v0, dt);
    vector4::addScaled(r1, r1, v1, dt);
    vector4::addScaled(r2, r2, v2, dt);
    vector4::addScaled(r3, r3, v3, dt);
  }
